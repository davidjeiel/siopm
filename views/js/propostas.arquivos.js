 ...
 $(function () {
 	...
 	//VER .0021
 	path_post			=  app_path + "/controllers/propostas.arquivos.controller.php";
            	...
            	if (up.files.length > 0){
					$.ajax({
						...
						data : {
							...
							PropostaArquivoTipoID : $("#PropostaArquivoTipoID option:selected").val(),
						...
						success : function(dados) {
							...
							if (dados.result == true){
		                        ...
		                        arr = new Array(
									...
									'<div class="PropostaArquivoDescricao">' + dados.propostaarquivodescricao + '</div>',
		                            links_html
		                        );
		                        ...
		                        var addId   = $('table#lista_arquivos').dataTable().fnAddData( arr );
		                        var tr      = $('table#lista_arquivos').dataTable().fnSettings().aoData[addId[0]].nTr;

		                        // Setamos os atributos do tr
		                        $(tr).addClass("alert propostaarquivo_" + dados.propostaarquivoid);
		                        $(tr).attr('data-arquivoid',  dados.arquivoid);
		                        $(tr).attr('data-propostaarquivoid',  dados.propostaarquivoid);
		                        $(tr).attr('data-arquivonome',  dados.filename);
		                        $(tr).attr('data-arquivodescricao', dados.propostaarquivodescricao);
						...
						error : function(jqXHR, textStatus, errorThrown) {
							progressError();
							alerta = ("ERRO propostas.file.js UploadComplete:\r\nNão foi possível carregar os dados\r\n\r\n("
