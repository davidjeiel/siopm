			...
			<table id="listaRelatorio" class="table table-condensed">
				<thead>
					<tr>
						...
						<?php echo $inversaoOrdemVisualizacaoRemuneracaoSelic; ?>
							...
							<?php
								...
								foreach( $headers as $eventotipoid => $value) {
									...
									if (isset($ativo["eventos"][$eventotipoid]) && ($ativo["eventos"][$eventotipoid] > 0))
									{
										...
										if ($eventotipoid != REMUNERACAO_SELIC) $totalEventos += $eventoValor;
						...
						<tr class='tr_total'>
							...
							<?php 
								...
								foreach($headers as $eventotipoid => $eventotipo): 
									if ($eventotipoid != COD_AMORTIZACAO && $eventotipoid != COD_AMORTIZACAO_EXTRAORDINARIA) {
										if ($eventotipoid == REMUNERACAO_SELIC) {
											$inversaoOrdemVisualizacaoRemuneracaoSelic =  "<td class='direita'><strong>" . PPOEntity::toMoneyBr($totaisEventos[$eventotipoid]) ."</strong></td>";
										}else echo "<td class='direita'><strong>" . PPOEntity::toMoneyBr($totaisEventos[$eventotipoid]) ."</strong></td>";
									}
								endforeach; 
							?>
